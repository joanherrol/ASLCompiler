...
if (Types.isArrayTy(t1)) {
    ...
    if (Symbols.isParameterClass(ctx->expr(0)->getText())) 
              ...
              ArrayL = ctx->expr(0)->getText();
    if (Symbols.isParameterClass(ctx->expr(1)->getText())) 
              code = code || instruction::LOAD(ArrayR, addr2);
    else         
              ArrayR = ctx->expr(1)->getText();

    //cogemos el tamaño de los arrays como string
    unsigned int si = Types.getArraySize(t1);
    std::string size = std::to_string(si);
    std::string tmp = "%"+codeCounters.newTEMP();
    std::string tmpI = "%"+codeCounters.newTEMP();
    std::string tmpSize = "%"+codeCounters.newTEMP();
    ...
    if (Types.isIntegerTy(t))
      ...
      code = code || instruction::FLOAD(temp, "0");
    code = code || instruction::ILOAD(tmpI, "0");
    code = code || instruction::ILOAD(tmp1, "1");
    code = code || instruction::ILOAD(tmpSize, size);
    ...
    instructionList codeWhile;
    std::string tmpCA = "%"+codeCounters.newTEMP();
    std::string tmpCB = "%"+codeCounters.newTEMP();
    std::string tmpMul = "%"+codeCounters.newTEMP();
    codeWhile = instruction::LOADX(tmpCB, ArrayR, tmpI);//tmp = b[i]
    codeWhile = codeWhile || instruction::LOADX(tmpCA, ArrayL, tmpI);//tmp = a[i]

    if (Types.isIntegerTy(t)) {
      codeWhile = codeWhile || instruction::MUL(tmpMul, tmpCA, tmpCB);//tmpMul = a[i] * b[i]
      codeWhile = codeWhile || instruction::ADD(temp, temp, tmpMul); //res += tmpMul
    }

    else {
      codeWhile = codeWhile || instruction::FMUL(tmpMul, tmpCA, tmpCB);//tmpMul = a[i] * b[i]
      codeWhile = codeWhile || instruction::FADD(temp, temp, tmpMul); //res += tmpMul
    }
