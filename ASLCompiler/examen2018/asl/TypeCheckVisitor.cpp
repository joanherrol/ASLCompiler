...
antlrcpp::Any TypeCheckVisitor::visitArithmetic(AslParser::ArithmeticContext *ctx) {
  ...
  else if (ctx->MUL() and (Types.isArrayTy(t1) or Types.isArrayTy(t2))) {
        //We check if both are arrays
        if (Types.isArrayTy(t1) and Types.isArrayTy(t2)) {

            //now that we know they are arrays, we get the size and element type
            TypesMgr::TypeId tE1 = Types.getArrayElemType(t1);
            TypesMgr::TypeId tE2 = Types.getArrayElemType(t2);


            unsigned int size1 = Types.getArraySize(t1);
            unsigned int size2 = Types.getArraySize(t2);
            ...
            if (size1 != size2) {
              ...
              if (Types.isIntegerTy(tE1) and Types.isIntegerTy(tE2)) {
                  t = Types.createIntegerTy();
              }     

              else if (Types.isFloatTy(tE1) and Types.isFloatTy(tE2)) {
...
antlrcpp::Any
TypeCheckVisitor::visitPairExpr(AslParser::PairExprContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->ident());
  ...
  if (not Types.isErrorTy(t1) and not Types.isPairTy(t1)) {
    TypesMgr::TypeId e = Types.createErrorTy();
    putTypeDecor(ctx, e);
    ...
    if (ctx->FIRST()) {
      TypesMgr::TypeId t = Types.getFirstPairType(t1);
      putTypeDecor(ctx, t);
    }
    else if (ctx->SECOND()) {
      TypesMgr::TypeId t = Types.getSecondPairType(t1);
      putTypeDecor(ctx, t);
    }
  }
...
antlrcpp::Any
TypeCheckVisitor::visitLeftPair(AslParser::LeftPairContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->ident());
