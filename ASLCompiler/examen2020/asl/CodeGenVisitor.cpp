...
antlrcpp::Any CodeGenVisitor::visitSumExpr(AslParser::SumExprContext *ctx) {
  DEBUG_ENTER();
  instructionList code; //el codi que ficarem a codAts
  std::string result = "%"+codeCounters.newTEMP();
  std::string aux = "%"+codeCounters.newTEMP();

  TypesMgr::TypeId tSum = getTypeDecor(ctx);

  if (Types.isFloatTy(tSum)) code = instruction::FLOAD(result, "0");
  else code = instruction::ILOAD(result, "0");

  for (unsigned int i = 0; i < ctx->expr().size(); ++i) {
    TypesMgr::TypeId tExpr = getTypeDecor(ctx->expr(i));
    CodeAttribs exprAts = visit(ctx->expr(i));
    code = code || exprAts.code;
    if (Types.isFloatTy(tSum) and Types.isIntegerTy(tExpr)) {
      code = code || instruction::FLOAT(aux, exprAts.addr);
      code = code || instruction::FADD(result, result, aux);
    }
    else if (Types.isFloatTy(tExpr)) code = code || instruction::FADD(result, result, exprAts.addr);
    else code = code || instruction::ADD(result, result, exprAts.addr);
  }
...
antlrcpp::Any CodeGenVisitor::visitFilterExpr(AslParser::FilterExprContext *ctx) {
  DEBUG_ENTER();
  instructionList code; //el codi que ficarem a codAts
  ...
  code = codAt1.code || codAt2.code;

      //First, we check if the arrays are parameters
    std::string ArrayL = "%"+codeCounters.newTEMP();
    std::string ArrayR = "%"+codeCounters.newTEMP();
    if (Symbols.isParameterClass(ctx->ident(0)->getText())) 
              code = code || instruction::LOAD(ArrayL, codAt1.addr);
    else 
              ArrayL = ctx->ident(0)->getText();
    if (Symbols.isParameterClass(ctx->ident(1)->getText()))
              code = code || instruction::LOAD(ArrayR, codAt2.addr);
    else         
              ArrayR = ctx->ident(1)->getText();

    //We get the array size as string
    unsigned int si = Types.getArraySize(getTypeDecor(ctx->ident(0)));
    std::string size = std::to_string(si);
    ...
    //while body------
    std::string arrayAccess = "%"+codeCounters.newTEMP();
    std::string funcResult = "%"+codeCounters.newTEMP();

    //we save the array value
    code = code || instruction::LOADX(arrayAccess, codAt1.addr, tmpI);
    
    //we call the function
    code = code || instruction::PUSH("");
    code = code || instruction::PUSH(arrayAccess);
    code = code || instruction::CALL(ctx->ident(2)->getText());
    code = code || instruction::POP("");  
    code = code || instruction::POP(funcResult);

    code = code || instruction::XLOAD(codAt2.addr, tmpI, funcResult);
    code = code || instruction::ADD(result, result, funcResult);
    code = code || instruction::ADD(tmpI, tmp1, tmpI); //i+1
    //----------------
  ...
  //we save function result in bool array and add 1 to the positives
  CodeAttribs codAts(result, "", code);
  DEBUG_EXIT();
  return codAts;
}
