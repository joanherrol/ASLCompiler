...
antlrcpp::Any
TypeCheckVisitor::visitSumExpr(AslParser::SumExprContext *ctx) {
  DEBUG_ENTER();
  visitChildren(ctx);
  ...
  for (unsigned int i = 0; i < ctx->expr().size(); ++i) {
    ...
    if (not Types.isErrorTy(t) and not Types.isNumericTy(t)) allNum = false;
...
antlrcpp::Any
TypeCheckVisitor::visitFilterExpr(AslParser::FilterExprContext *ctx) {
  DEBUG_ENTER();
  visitChildren(ctx);
  TypesMgr::TypeId tId1, tId2, tId3;
  
  tId1 = getTypeDecor(ctx->ident(0));
  tId2 = getTypeDecor(ctx->ident(1));
  tId3 = getTypeDecor(ctx->ident(2));

  bool b1, b2, b3, b4;
  b1 = (not Types.isArrayTy(tId1)) or (not Types.isArrayTy(tId2));
  b2 = (Types.isArrayTy(tId2) and (not Types.isBooleanTy(Types.getArrayElemType(tId2))));
  b3 = not (Types.isArrayTy(tId1) and Types.isArrayTy(tId2) and (Types.getArraySize(tId1) == Types.getArraySize(tId2)));
  b4 = not (Types.isFunctionTy(tId3) and Types.isBooleanTy(Types.getFuncReturnType(tId3)) and (Types.getNumOfParameters(tId3) == 1) and (Types.isArrayTy(tId1) and (Types.equalTypes(Types.getArrayElemType(tId1), Types.getParameterType(tId3, 0)))));
  
  if (b1 or b2 or b3 or b4) Errors.incompatibleOperator(ctx->op);
