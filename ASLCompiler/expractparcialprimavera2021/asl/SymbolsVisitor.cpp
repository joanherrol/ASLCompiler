...
antlrcpp::Any SymbolsVisitor::visitFunction(AslParser::FunctionContext *ctx) {
  ...
  if (Symbols.findInCurrentScope(ident)) {
    ...
    if (ctx->parameters()) {
        for (auto ct:ctx->parameters()->type()) {
...
antlrcpp::Any SymbolsVisitor::visitFuncType(AslParser::FuncTypeContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->basicType());
...
antlrcpp::Any SymbolsVisitor::visitArrayDecl(AslParser::ArrayDeclContext *ctx) {
  DEBUG_ENTER();
  unsigned int size = std::stoi(ctx->INTVAL()->getText());
  visit(ctx->basicType());
  TypesMgr::TypeId bt = getTypeDecor(ctx->basicType());
...
antlrcpp::Any SymbolsVisitor::visitTupleDecl(AslParser::TupleDeclContext *ctx) {
  DEBUG_ENTER();
  visitChildren(ctx);
  std::vector<TypesMgr::TypeId> paramsTy;
    for (auto ct:ctx->basicType()) {
        paramsTy.push_back(getTypeDecor(ct));
    }
  TypesMgr::TypeId t = Types.createTupleTy(paramsTy);
  putTypeDecor(ctx, t);
  DEBUG_EXIT();
  return 0;
}
