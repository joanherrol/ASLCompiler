...
antlrcpp::Any
TypeCheckVisitor::visitPackStmt(AslParser::PackStmtContext *ctx) {
DEBUG_ENTER();
  visitChildren(ctx);
  TypesMgr::TypeId t1 = getTypeDecor(ctx->ident());
  int size;
  if (not Types.isErrorTy(t1) and not Types.isArrayTy(t1))
    ...
    if (ctx->expr().size() != size)     Errors.packUnpackNumberOfItemsMismatch(ctx);
      TypesMgr::TypeId elemT = Types.getArrayElemType(t1);
      for (unsigned int i = 0; i < ctx->expr().size(); ++i) {
        TypesMgr::TypeId t = getTypeDecor(ctx->expr(i));
        if (not Types.isErrorTy(t) and not (Types.isIntegerTy(t) and Types.isFloatTy(elemT))) {
          if (not Types.equalTypes(t, elemT)) Errors.packUnpackIncompatibleTypes(ctx, i+1);
        }
      }
    }
  DEBUG_EXIT();
...
antlrcpp::Any
TypeCheckVisitor::visitUnpackStmt(AslParser::UnpackStmtContext *ctx) {
DEBUG_ENTER();
  visitChildren(ctx);
  TypesMgr::TypeId t1 = getTypeDecor(ctx->ident());
  int size;
  if (not Types.isErrorTy(t1) and not Types.isArrayTy(t1))
    ...
    if (ctx->expr().size() != size)     Errors.packUnpackNumberOfItemsMismatch(ctx);
      ...
      for (unsigned int i = 0; i < ctx->expr().size(); ++i) {
        TypesMgr::TypeId t = getTypeDecor(ctx->expr(i));
        if (not Types.isErrorTy(t) and not (Types.isIntegerTy(elemT) and Types.isFloatTy(t))) {
          if (not Types.equalTypes(t, elemT)) Errors.packUnpackIncompatibleTypes(ctx, i+1);
        }
      }
    }
...
antlrcpp::Any TypeCheckVisitor::visitLeftTuple(AslParser::LeftTupleContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->ident());
  unsigned int i = std::stoi(ctx->INTVAL()->getText());
  bool b = getIsLValueDecor(ctx->ident());
  putIsLValueDecor(ctx, b);
  ...
  if (not Types.isErrorTy(t1) and not Types.isTupleTy(t1)) {
    ...
    unsigned int size = Types.getTupleSize(t1);
    if (size <= i) Errors.nonExistentFieldInTuple(ctx);
    else putTypeDecor(ctx, Types.getTupleFieldType(t1, i));
  }
...
antlrcpp::Any TypeCheckVisitor::visitExprTuple(AslParser::ExprTupleContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->ident());
  unsigned int i = std::stoi(ctx->INTVAL()->getText());
  bool b = getIsLValueDecor(ctx->ident());
